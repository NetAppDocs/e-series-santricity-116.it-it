...
Compatibile con DULBE
 a| 
Indica se il disco dispone dell'opzione Deallocated (disallocato) o Unwritten Logical Block Error (DULBE). DULBE è un'opzione sui dischi NVMe che consente a uno storage array EF600 di deallocare i blocchi che fanno parte di un volume. La deallocazione dei blocchi su un disco può ridurre notevolmente il tempo necessario per inizializzare i volumi. Inoltre, gli host possono deallocare i blocchi logici nel volume utilizzando il comando NVMe Dataset Management.
